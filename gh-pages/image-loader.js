...
async function loadFromIgmeCache(site) {
    ...
    const hash = md5(site.url);
    const jsonPath = `data/cache/igme_html/${hash}.json`;
    
    console.log('Looking for cached images at:', jsonPath);
    
    try {
        const response = await fetch(jsonPath);
        if (response.ok) {
            const data = await response.json();
            console.log('Found', data.images?.length || 0, 'images with descriptions');
            return data.images || [];
        } else {
            console.log('Cache file not found:', response.status);
        }
    } catch (e) {
        console.log('Error loading cached IGME metadata for', site.code, ':', e.message);
    }
 ...
 * Uses a simple MD5 implementation since crypto.subtle doesn't support MD5
 * @param {string} str - String to hash
 * @returns {string} MD5 hash
 */
function md5(str) {
    // Simple MD5 implementation for browser
    // Based on: https://stackoverflow.com/questions/14733374/how-to-generate-an-md5-hash-in-javascript
